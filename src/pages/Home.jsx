...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-lg text-slate-400">
              Trabajamos con las principales instituciones, empresas y organizaciones de Costa Rica
            </p>
          </motion.div>
            ...
            {CLIENTS.map((client, index) => (
              ...
              >
                <img
                  ...
                  onError={(e) => {
                    console.error('[ERROR] Failed to load logo for:', client.name);
                    e.target.onerror = null;
                    e.target.src = 'https://sensible-spoonbill-485.convex.cloud/api/storage/be58cd4c-27a9-4bdd-9ff5-2e4b8a252aeb';
                  }}
                />
              </motion.div>
            ))}
      ...
      {/* Featured Blog Section with D'GEO Branding */}
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-orange-600 via-orange-700 to-amber-600">
        <div className="max-w-7xl mx-auto">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            <h2 className="text-4xl font-bold text-white mb-4">
              Blog de Conocimiento Técnico
            </h2>
            <p className="text-lg text-orange-100 max-w-2xl mx-auto">
              Artículos técnicos, casos de estudio y análisis especializados en ingeniería geotécnica
            </p>
          </motion.div>
          ...
          {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                {blogPosts.map((post, index) => (
                  ...
                  >
                    <Link
                      to={`/blog/${post.slug}`}
                      className="block h-full bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all transform hover:-translate-y-2"
                    >
                      {post.featured_image && (
            ...
            <div className="text-center py-12">
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-8 inline-block">
                <p className="text-white text-lg">No hay publicaciones disponibles en este momento.</p>
              </div>
            </div>
          )}
        </div>
